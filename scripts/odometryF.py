...
class localisation:
    def __init__(self , mode):
        ...
        self.x = rospy.get_param("robot_X", 0.0)
        self.y = rospy.get_param("robot_Y", 0.0)
        self.theta = rospy.get_param("robot_THETA", 0.0)
        
        self.orientation = Float32()        
        
        self.odom = Odometry()
        self.covariance = dead_reckoning(self.dt , self.x , self.y , self.theta)
        ...
        while not rospy.is_shutdown():
            ...
            self.orientation = u[2]

            ###--- Publish ---###
            self.odom_pub.publish(self.odom)
            self.robot_orientation_pub.publish(self.orientation)
            rate.sleep() 
